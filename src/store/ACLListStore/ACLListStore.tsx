...
import { ACLListDataProps } from "./ACLListInterface";

export default class ACLListStore {
  agGrid: any = null;
  ...
  setEditValues = (data: any) => {
    ...
    return axios.post(API_URL.ACLLIST.VIEW + "/" + data.id).then(({ data }) => {
  ...
  public fetchList = async (): Promise<any> => {
    this.setListData([]);
    return await axios
      .post(API_URL.ACLLIST.LIST)
      .then(({ data }) => {
        this.setListData(data.acl_list);
      })
      .catch((response) => {
  ...
  AddData = (formData: any) => {
    return axios
      .post(API_URL.ACLLIST.ADD, formData)
      .then(() => {
  ...
  DeleteData = (id: number) => {
    return axios
      .delete(API_URL.ACLLIST.DESTROY + `/${id}`)
      .then(() => {
        return this.fetchList();
      })
      .catch((response) => {
  ...
  EditData = (formData: any, editId: number) => {
    return axios
      .post(API_URL.ACLLIST.EDIT + "/" + editId, formData)
      .then(() => {
        return this.fetchList();
      })
      .catch((response) => {
  ...
  ViewData = (id: number, type: string) => {
    return axios
      .post(API_URL.ACLLIST.VIEW + "/" + id, type)
      .then(({ data }) => {
