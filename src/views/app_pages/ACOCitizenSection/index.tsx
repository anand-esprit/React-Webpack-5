...
import { ACOCitizenSectionBreadcrumb } from "../../../config/BreadcrumbConfig";
...
const ACOCitizenSection: React.FC = observer(() => {
  const { ACL_CITIZEN_SECTION_STORE } = useStore();
  const {
    list_data,
    fetchList,
    setDeleteValues,
  ...
  return (
    <>
      <BreadcrumbComponent items={ACOCitizenSectionBreadcrumb.path} />
      <div className="mainPageWrap">
        <PageHeader
          className="mainPageHeader"
          title={ACOCitizenSectionBreadcrumb.title}
          extra={[
...
export default ACOCitizenSection;
