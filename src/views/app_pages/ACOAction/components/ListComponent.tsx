...
const ListComponent: React.FC<any> = observer(
  ({ rowData, getData, openDeleteModal, openEditModal }) => {
    const { AUTH, ACO_ACTION_STORE } = useStore();
    const { setupGrid } = ACO_ACTION_STORE;
    ...
    return (
      ...
      >
        ...
        <AgGridColumn field="action" headerName="Action" />
        <AgGridColumn field="controller" headerName="Controller" />
        <AgGridColumn field="controller_name" headerName="Controller Name" />
        <AgGridColumn
          headerName="Actions"
          type="actionColumn"
          sortable={false}
