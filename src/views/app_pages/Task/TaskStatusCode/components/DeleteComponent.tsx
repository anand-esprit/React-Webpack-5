...
const DeleteComponent: React.FC<any> = observer(({ visible, close }) => {
  const { TASK_STATUS_CODE_STORE, ROOT } = useStore();
  const { DeleteData, deleteValues } = TASK_STATUS_CODE_STORE;
  ...
  return deleteValues ? (
    ...
    >
      <div className="deleteNote">
        Are you sure want to delete <span>{deleteValues.status_name}</span> Task
        Status Code?
      </div>
    </Modal>
  ) : (
