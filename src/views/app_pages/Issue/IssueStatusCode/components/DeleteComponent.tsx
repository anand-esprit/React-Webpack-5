...
const DeleteComponent: React.FC<any> = observer(({ visible, close }) => {
  const { ISSUE_STATUS_CODE_STORE, ROOT } = useStore();
  const { DeleteData, deleteValues } = ISSUE_STATUS_CODE_STORE;
  ...
  return deleteValues ? (
    ...
    >
      <div className="deleteNote">
        Are you sure want to delete <span>{deleteValues.name}</span> Issue
        Status Code?
      </div>
    </Modal>
  ) : (
